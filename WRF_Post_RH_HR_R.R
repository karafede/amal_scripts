...
import_nc_WRF <- function(filenames){
  ...
  for(j in 1:length(filenames)) {
    ...
    #### only one variable (HR 2m) == var = 18
...
writeRaster(BBB, "RH_2060_May_Original_Hourly.tif" , options= "INTERLEAVE=BAND", overwrite=T)
...
writeRaster(BBB, "RH_2060_May_LC_Hourly.tif" , options= "INTERLEAVE=BAND", overwrite=T)
...
RH_original <- stack("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Original/RH_2060_May_Original_Hourly.tif")
Mean_RH_original <- mean(RH_original)
#MAX_RH_original <- max(RH_original)
#MIN_RH_original <- min(RH_original)
plot(Mean_RH_original)
#plot(MAX_RH_original)
#plot(MIN_RH_original)

RH_LC <- stack("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/LC/RH_2060_May_LC_Hourly.tif")
Mean_RH_LC <- mean(RH_LC) #we have to work on one operator at a time
#MAX_RH_LC <- max(RH_LC) #we have to work on one operator at a time
#MIN_RH_LC <- min(RH_LC) #we have to work on one operator at a time
plot(Mean_RH_LC) #we have to work on one operator at a time
#plot(MAX_RH_LC) #we have to work on one operator at a time
#plot(MIN_RH_LC) #we have to work on one operator at a time


Diff_LCO2_mean <- Mean_RH_LC - Mean_RH_original
plot(Diff_LCO2_mean)
writeRaster(Diff_LCO2_mean, "C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference_mean_RH_2060_May.tif" , options= "INTERLEAVE=BAND", overwrite=T)



Diff_LCO2_min <- MIN_RH_LC - MIN_RH_original
plot(Diff_LCO2_min)
writeRaster(Diff_LCO2_min, "C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference2_min_RH_2060_May.tif" , options= "INTERLEAVE=BAND", overwrite=T)



Diff_LCO2_max <- MAX_RH_LC - MAX_RH_original
plot(Diff_LCO2_max)
writeRaster(Diff_LCO2_max, "C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference2_max_RH_2060_May.tif" , options= "INTERLEAVE=BAND", overwrite=T)
...
diff_map <- raster("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference_mean_RH_2060_May.tif")
# diff_map <- raster("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference2_min_RH_2060_May.tif")
# diff_map <- raster("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference2_max_RH_2060_May.tif")
...
# cool = rainbow(25, start=rgb2hsv(col2rgb('gold'))[1], end=rgb2hsv(col2rgb('royalblue2'))[1])
# cool_2 = rainbow(25, start=rgb2hsv(col2rgb('royalblue2'))[1], end=rgb2hsv(col2rgb('azure'))[1])
# warm = rainbow(150, start=rgb2hsv(col2rgb('green'))[1], end=rgb2hsv(col2rgb('yellow'))[1])
# cols = c(rev(cool), rev(cool_2), rev(warm))


cool = rainbow(180, start=rgb2hsv(col2rgb('yellow'))[1], end=rgb2hsv(col2rgb('blue'))[1])
...
RH_images <- raster("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference_mean_RH_2060_May.tif")
# Temp_images <- raster("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference_min_RH_2060_May.tif")
# Temp_images <- raster("C:/Users/aaldababseh/WRF_PostProcessing_DATA/Test2_May_LC/Difference_max_RH_2060_May.tif")

plot(RH_images)

h <- rasterVis::levelplot(RH_images, 
                          margin=FALSE, main= "RH_mean difference in land cover",
                          #    margin=FALSE, main= "RH_min difference in land cover",
                          #   margin=FALSE, main= "RH_max difference in land cover",
                          xlab = "",
                          ylab = "",
                          ## about colorbar
                          colorkey=list(
                          ...
                          names.attr=rep(names(RH_images))) 
#  latticeExtra::layer(sp.polygons(shp_ME))
h

png(paste0(output_folder,"RH_mean_diff_land_cover.png"), width = 900, height = 900,
...
# png(paste0(output_folder,"RH_min_diff_land_cover.png"), width = 900, height = 900,
#     units = "px", pointsize = 50,
#     bg = "white", res = 200)
# print(h)
# dev.off()
...
# png(paste0(output_folder,"RH_max_diff_land_cover.png"), width = 900, height = 900,
#     units = "px", pointsize = 50,
#     bg = "white", res = 200)
# print(h)
# dev.off()
